...
arm1 = Dynamics.euler2R([phi theta psi]) * [arm1; zeros(1,2)] + [[x y z].' [x y z].'];
arm2 = Dynamics.euler2R([phi theta psi]) * [arm2; zeros(1,2)] + [[x y z].' [x y z].'];
% arm3 = vec2RotMat(phi,theta,psi)*[arm3; zeros(1,2)] + [[x y z].' [x y z].'];
% arm4 = vec2RotMat(phi,theta,psi)*[arm4; zeros(1,2)] + [[x y z].' [x y z].'];

objects{1} = plot3(arm1(1,:),arm1(2,:),arm1(3,:),'-k','LineWidth',1.5); hold on;
objects{2} = plot3(arm2(1,:),arm2(2,:),arm2(3,:),'-k','LineWidth',1.5);
