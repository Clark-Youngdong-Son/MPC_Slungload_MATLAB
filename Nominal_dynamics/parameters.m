...
x_final = zeros(n,1);
maxIter = 300;
x_trajec_optimal = zeros(n,N+1,maxIter);
u_trajec_optimal = zeros(m,N, maxIter);
...
dynamics_param.dx = 0.0001;
dynamics_param.du = 0.0001;
dynamicsGradient = @Dynamics.dynamicsGradient;

%Cost
cost = @Cost.cost;
cost_param.x_f = x_final;
cost_param.L = diag([3000 3000 13500 3000 3000 3000 200 200 200 50 50 50 100 100 100 1 1 0]);
cost_param.Q = diag([100 100 900 1000 1000 1000 100 100 100 60 60 100 80 80 160 1 1 0]);
cost_param.R = diag([10 60 60 240]);
cost_param.u_ref = [(2.45 + 0.05)*9.81 0 0 0].';
cost_param.N = N;
cost_param.dx = 0.0001;
cost_param.du = 0.0001;
costOne = @Cost.costOne;
costGradient = @Cost.costGradient;
cost_param.obstacle_enable = false;
if cost_param.obstacle_enable
    cost_param.obstacle_consier = 'xy';
%     cost_param.obstacle_consier = 'xyz';
end
