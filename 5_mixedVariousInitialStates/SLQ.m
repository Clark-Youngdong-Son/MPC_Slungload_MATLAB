function [x_nominal, u_nominal, time, cost_new] = SLQ(x_now, x_trajec, param)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
tic;
param.cost_param.x_ref = x_trajec;
costDecreased = false;
%Main function
x_nominal(:,1) = x_now;
...
if((param.warmstart_original || param.warmstart_modified) && ~isempty(x_trajec)) %With warmstart
    ...
    for i=1:param.N
        u_nominal(:,i) = param.nominal_controller(x_nominal(:,i), [x_trajec(1:3,i); x_trajec(10:12,i); zeros(3,1)], param.nominal_controller_param);
%         u_nominal(:,i) = param.nominal_controller(x_nominal(:,i), [x_trajec(1:3,end); zeros(6,1)], param.nominal_controller_param);
        x_nominal(:,i+1) = param.dynamics(x_nominal(:,i), u_nominal(:,i), param.dynamics_param);
    end
end
...
for i=1:param.maxIter %SLQ iteration
    ...
    s1 = param.cost_param.L*(x_nominal(:,end)-x_trajec(:,end));
    ...
    for j=param.N:-1:1
        ...
        [q1, r1, Q2, R2] = param.costGradient(x_nominal(:,j), u_nominal(:,j), param.costOne, param.cost_param, j);
    ...
    if(cost_temp<cost_old)
        ...
        if(costDecreased)
            disp('Not decreasing anymore');
            break;
        else
            disp('Cost does not decrease at all');
        end
    end
end
time = toc;
