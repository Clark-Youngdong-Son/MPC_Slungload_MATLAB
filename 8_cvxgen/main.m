...
for i=1:initialNumber
    cvx_begin
        ...
        A_eq*u == b_eq
%         for k=1:N
%             exp(norm((A_o(18*(k-1)+1:18*k,:)*u+B_o(18*(k-1)+1:18*k,:)*x_initial))) <= 0.3
%             exp(norm(u)) <= 1
%         end
        k=1*N/4;
        fullState = A_o(18*(k-1)+1:18*k,:)*u+B_o(18*(k-1)+1:18*k,:)*x_initial
        norm(fullState(1:3) - [0.5 0.5 0].') <= 0.3
        
        k=2*N/4;
        fullState = A_o(18*(k-1)+1:18*k,:)*u+B_o(18*(k-1)+1:18*k,:)*x_initial
        norm(fullState(1:3) - [0.5 0 0].') <= 0.3
...
%     temp = quadprog(QP_param.H, [], [], [], QP_param.A_eq, QP_param.b_eq, [], [], [], QP_param.option);
    u_flat_trajec_optimal(:,:,i) = reshape(u, 3, N);
