...
%     pqr = zeros(3,1);
%     pqr(1) = -dot((m_Q/f*x_Q_dot3 - f_dot1/f*b3 - (T_dot1*p + T*p_dot1)/f),b2);
%     pqr(2) = dot((m_Q/f*x_Q_dot3 - f_dot1/f*b3 - (T_dot1*p + T*p_dot1)/f),b1);
%     pqr(3) = dot(psi_dot1*e3 + theta_dot1*c2,b3);
...
%         alpha = zeros(3,1);
%         alpha(1) = -dot((m_Q*x_Q_dot4  - 2*f_dot1*cross(pqr,b3) - f*cross(pqr,cross(pqr, b3)) -(T_dot2*p + 2*T_dot1*p_dot1 + T*p_dot2)),b2);
%         alpha(2) = dot((m_Q*x_Q_dot4  - 2*f_dot1*cross(pqr,b3) - f*cross(pqr,cross(pqr, b3)) -(T_dot2*p + 2*T_dot1*p_dot1 + T*p_dot2)),b1);
%         alpha(3) = dot(psi_dot2*e3 + cross(psi_dot1*e3,theta_dot1*c2) + theta_dot2*c2,b3);
        alpha = Dynamics.pqr2RpyDot([phi_dot2 theta_dot2 psi_dot2].', euler);
%         alpha = psi_dot2*e3 + cross(psi_dot1*e3, theta_dot1*c2+phi_dot1*b1) + theta_dot2*c2 + cross(theta_dot1*c2, phi_dot1*b1) + phi_dot2*b1;
%         alpha = psi_dot2*e3 + cross(psi_dot1*e3, theta_dot1*c2+phi_dot1*b1) + theta_dot2*c2 + phi_dot2*b1 + phi_dot1*psi_dot1*cross(e3,c2) + phi_dot1*cross(psi_dot1*e3+theta_dot1*c2,b1);
